...
以上都是直接或者间接实现run方法，调用thread.start()启动线程,线程会去执行run()方法
##2.6 如何通过Thread 类创建线程？
...
如果说上面的说法让你感到困惑，那么我举个例子，假设 Thread2 中 nwait(ew 了Thread1，main 函数中 new 了 Thread2，那么:
...
阻塞状态再细分：
![alt](线程状态2.png)
NEW,RUNNABLE,BLOCKED,TERMINATED/DEAD
NEW -- 尚未启动    
RUNNABLE --正在执行中  
BLOCKED--阻塞的（被同步锁或者IO锁阻塞）  
TERMINATED/DEAD--执行完成  
...
ThreadPoolExecutor()【重要】 是最原始的线程池创建  
...
上面1-3创建方式都是对ThreadPoolExecutor的封装。
...
原文链接：https://blog.csdn.net/lupengfei1009/article/details/113404925