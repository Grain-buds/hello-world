...
#### char类型能存储汉字？
unicode 编码 占用2个字节，char 也是2个字节，可以存储。
...
2、==：是操作符。
二、比较的对象不同
1、equals()：用来检测两个对象是否相等，即两个对象的内容是否相等，不能用于比较基本数据类型。
...
https://blog.csdn.net/weixin_46460843/article/details/110310604
...
1、在计算机系统中，浮点数采用 符号+阶码+尾数 进行表示
2、无法使用==或者包装类型的equals()来精确比较，尽管无法做到精确比较，但是我们可以确定一个误差范围，当小于这个误差范围的时候就认为这两个浮点数相等
3、BigDecimal，BigDecimal 是不可变的，它能够精确地表示十进制数字。需要注意的是，创建 BigDecimal 对象时，要使用参数为 String 的构造方法，不要使用构造参数为 double 的，如果非要使用 double 创建，一定要用 valueOf 静态方法，防止丢失精度。然后调用 compareTo 方法比较即可
https://blog.csdn.net/qq_34626094/article/details/121188058
...
类型擦除：java编译器生成的字节码不包含泛型信息，所以在编译时擦除。主要过程就是： 首先将所有的泛型参数用其最顶级的父类型替换，然后移除所有的类型参数。
